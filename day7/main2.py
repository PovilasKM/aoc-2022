...
def get_all_sizes(top_dir):
    total = [top_dir.size]
    for directory in top_dir.directories:
        total += get_all_sizes(directory)
    return total
...
unused_space = 70000000 - top_dir.size
required_space = 30000000 - unused_space
print(required_space)
print(min([x for x in get_all_sizes(top_dir) if x > required_space]))
